...
export default async function ToolPage({ params }: Props) {
  ...
  const pricingMap: Record<string, { label: string; color: string }> = {
    ...
    freemium: { label: 'Free+Paid', color: 'text-yellow-400 bg-yellow-500/10 border border-yellow-500/20' },
    paid: { label: 'Paid', color: 'text-red-400 bg-red-500/10 border border-red-500/20' },
  };
        ...
        <main className="max-w-5xl mx-auto px-4 py-10">
            ...
            <div className="space-y-6">
              ...
              <div className="card-base p-6 gradient-border">
                ...
                <div className="space-y-3">
                  ...
                  >
                    Found a better tool? Recommend it →
                  </a>
                </div>
              </div>
          ...
          {alternatives.length > 0 && (
            <section className="mt-12">
              <h2 className="text-xl font-bold mb-6 flex items-center gap-2">
                <span>🔄</span> {tool.name} Alternatives
              </h2>
          ...
          <section className="mt-12 card-base p-6 gradient-border">
            <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
              <span>💡</span> Why Choose {tool.name}？
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="p-4 bg-gray-800/50 rounded-lg">
                <div className="text-2xl mb-2">🎯</div>
                <h3 className="text-sm font-bold text-gray-200 mb-1">Precisely Matches Your Needs</h3>
                <p className="text-xs text-gray-400">{tool.name} excels at {tool.useCases.slice(0, 2).join(', ')}.</p>
              </div>
              <div className="p-4 bg-gray-800/50 rounded-lg">
                <div className="text-2xl mb-2">⭐</div>
                <h3 className="text-sm font-bold text-gray-200 mb-1">Great Rating {tool.score}/10</h3>
                <p className="text-xs text-gray-400">{tool.name} scores high.Core strengths: {tool.prosCons.pros.slice(0, 2).join(', ')}.</p>
              </div>
              <div className="p-4 bg-gray-800/50 rounded-lg">
                <div className="text-2xl mb-2">💪</div>
                <h3 className="text-sm font-bold text-gray-200 mb-1">Wide Range of Use Cases</h3>
                <p className="text-xs text-gray-400">Ideal for {tool.useCases.slice(0, 4).join(', ')} and more</p>
              </div>
            </div>
          </section>
